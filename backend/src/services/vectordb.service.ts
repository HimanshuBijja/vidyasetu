...
export class VectorDBService {
  ...
  async initCollection(collectionName?: string): Promise<Collection> {
    try {
      ...
      throw new Error(
        `Failed to initialize vector database collection: ${error.message || error}`,
      );
    }
  }
  ...
  async storeChunks(
    ...
    try {
      ...
      const chunksByPdf = chunks.reduce(
        (acc, chunk, index) => {
        ...
        {} as Record<
          string,
          { chunks: Chunk[]; embeddings: number[][]; fileName: string }
        >,
      );
      ...
      console.error("ChromaDB storeChunks error:", error.message || error);
      throw new Error(
        `Failed to store chunks in vector database: ${error.message || error}`,
      );
    }
  }
  ...
  async queryChunks(
    ...
    try {
      ...
      throw new Error(
        `Failed to query vector database: ${error.message || error}`,
      );
    }
  }
  ...
  async queryChunksWithFilter(
    ...
    try {
      ...
      throw new Error(
        `Failed to query vector database with filter: ${error.message || error}`,
      );
    }
  }
  ...
  async queryByMetadata(
    ...
    try {
      ...
      throw new Error(`Failed to query by metadata: ${error.message || error}`);
    }
  }
  ...
  async deleteByFileId(fileId: string, collectionName?: string): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to delete chunks from vector database: ${error.message || error}`,
      );
    }
  }
  ...
  async getStats(): Promise<{ count: number }> {
    try {
      ...
      throw new Error(
        `Failed to get collection stats: ${error.message || error}`,
      );
    }
  }
  ...
  async getAllDocuments(
    ...
    try {
      ...
      throw new Error(`Failed to get all documents: ${error.message || error}`);
    }
  }
  ...
  async getUniqueFiles(
    ...
    try {
      ...
      console.error("ChromaDB getUniqueFiles error:", error.message || error);
      throw new Error(`Failed to get unique files: ${error.message || error}`);
    }
  }
  ...
  async getDocumentsByFileId(
    ...
    try {
      ...
      throw new Error(
        `Failed to get documents by file ID: ${error.message || error}`,
      );
    }
  }
