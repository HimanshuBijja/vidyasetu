...
export class DocumentService {
  ...
  async storePages(
    ...
    try {
      // Skip MongoDB storage if not connected (upload continues with ChromaDB + file storage)
      if (mongoose.connection.readyState !== 1) {
        console.warn(
          `⚠️  MongoDB not connected. Skipping page storage for ${fileName} (ChromaDB + file storage still work).`,
        );
        return;
      }
  ...
  async storeDocument(
    ...
    try {
      ...
      if (mongoose.connection.readyState !== 1) {
        console.warn(
          `⚠️  MongoDB not connected. Skipping document storage for ${fileName} (ChromaDB + file storage still work).`,
        );
        return;
      }
