...
export class ChatService {
  ...
  async getOrCreateSession(
    ...
    try {
      ...
      // Atomic upsert: find or create in a single operation (no race condition)
  ...
  async updateChatName(
    ...
    try {
      ...
      console.log(
        `📝 Chat name updated: "${chatName}" (${userId}/${sessionId})`,
      );
    } catch (error) {
  ...
  async deleteSession(userId: string, sessionId: string): Promise<void> {
    try {
      ...
      console.log(
        `🗑️  Chat session deleted from MongoDB (${userId}/${sessionId})`,
      );
      ...
      try {
        ...
        console.log(
          `🗑️  Session files deleted from disk (${userId}/${sessionId})`,
        );
      } catch (fileError) {
      ...
      if (chromaCollectionName) {
        try {
          ...
          console.log(
            `🗑️  ChromaDB collection deleted: ${chromaCollectionName}`,
          );
        } catch (chromaError) {
  ...
  async saveMessageTranslation(
    ...
    try {
      ...
      if (messageIndex !== -1) {
        ...
        await ChatHistoryModel.updateOne({ userId, sessionId }, updateQuery);
      } else {
        console.warn(
          `⚠️ Message not found for translation update (${userId}/${sessionId})`,
        );
      }
    } catch (error) {
